...
import { PumpAmmSdk, canonicalPumpPoolPda, sellBaseInputInternal } from '@pump-fun/pump-swap-sdk';
...
export async function getPumpSwapPrice(connection: Connection, mint: PublicKey): Promise<MarketPriceResult> {
	...
	// User address not needed for state fetch; use pool PDA as placeholder
	const swapState = await sdk.swapSolanaState(poolKey, poolKey);
